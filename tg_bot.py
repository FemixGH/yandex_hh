...
def format_bartender_response(text: str) -> str:
    ...
    # Теперь применяем форматирование (используем уже экранированный текст)

    # Заголовки рецептов (убираем жирное форматирование)
    # Ищем строки, которые начинаются с заглавной буквы и могут заканчиваться двоеточием
    # text = re.sub(r'^([А-ЯЁA-Z][^:\n]*):?\s*$', r'*\1*', text, flags=re.MULTILINE)

    # Ингредиенты (ищем уже экранированные дефисы)
    text = re.sub(r'^\\- (.+)$', r'• _\1_', text, flags=re.MULTILINE)

    # Шаги приготовления (нумерованные списки с экранированными точками, убираем жирное форматирование)
    ...
    for drink in drinks:
        # Экранируем название напитка для поиска
        escaped_drink = escape_markdown_v2(drink)
        pattern = r'\b(' + re.escape(escaped_drink) + r')\b'
        text = re.sub(pattern, r'*\1*', text, flags=re.IGNORECASE)

    # Выделяем температуры и время (ищем уже экранированные символы)
    text = re.sub(r'\b(\d+\s*°C|\d+\s*градус|\d+\s*мин|\d+\s*сек)\b', r'`\1`', text)

    # Выделяем количества ингредиентов (ищем уже экранированные точки)
